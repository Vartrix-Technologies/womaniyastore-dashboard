...
export function SizesTab({ shopId, sizes, onRefresh }: SizesTabProps) {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    const target = sizes[targetIndex];
    try {
      await Promise.all([
        supabase.from('sizes').update({ sort_order: target.sort_order }).eq('id', current.id),
        supabase.from('sizes').update({ sort_order: current.sort_order }).eq('id', target.id),
      ]);
      await onRefresh();
    } catch {
