...
import { calculateDateRange, buildPaginationRange, getPaginationInfo, type DateFilterType, type DateRange, type PaginationInfo } from '@/lib/utils';
 ...
 * Combines filter state with the calculateDateRange utility.
 * 
 * @example
 * const { dateFilter, setDateFilter, dateRange, startDateISO } = useDateFilter();
...
export function useDateFilter(options: UseDateFilterOptions = {}): UseDateFilterReturn {
  ...
  const dateRange = useMemo(() => calculateDateRange(dateFilter), [dateFilter]);
