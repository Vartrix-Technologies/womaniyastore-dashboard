...
const CART_RESTORED_KEY = 'womaniya-pos-cart-restored';
...
export default function POSPage() {
  ...
  useEffect(() => {
    ...
    if (savedCart.length > 0) {
      ...
      // Only show toast once per browser session (not on every tab navigation)
      const alreadyRestored = sessionStorage.getItem(CART_RESTORED_KEY);
      if (!alreadyRestored && !hasShownRestoreToast.current) {
        hasShownRestoreToast.current = true;
        sessionStorage.setItem(CART_RESTORED_KEY, 'true');
