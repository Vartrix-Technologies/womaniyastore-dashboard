...
export default function POSPage() {
  ...
  const handleAddFromSearch = (item: CartItem) => {
    if (cart.some((i) => i.qrCode === item.qrCode)) {
      toast.error('Item already in cart');
      return;
    }
  ...
  const handleRemoveItem = (qrCode: string) => {
    setCart(cart.filter((i) => i.qrCode !== qrCode));
  ...
  const handleUpdateDiscountReason = (qrCode: string, reason: string) => {
    setCart(
      cart.map((item) =>
  ...
  const handleUpdateSaleType = (qrCode: string, soldOnSale: boolean, saleType?: string) => {
    setCart(
      cart.map((item) =>
