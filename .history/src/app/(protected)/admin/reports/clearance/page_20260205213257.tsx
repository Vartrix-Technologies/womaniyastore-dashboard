...
import { ArrowLeft, TrendingDown, DollarSign, Package, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
...
export default function ClearanceReportPage() {
  ...
  const loadClearanceData = async () => {
    try {
      ...
      // Get ALL lots marked for clearance (inventory-centric approach)
      ...
      lots.forEach((lot: any) => {
        ...
        (lot.inventory_items || []).forEach((item: any) => {
          ...
          const profitLoss = isSold ? soldData.sold_price - costPrice : undefined;
          ...
          if (isSold) {
            ...
            catSummary.cost += costPrice;
            catSummary.profitLoss += profitLoss!;
          } else {
            catSummary.unsold++;
          }
        });
  ...
  const soldItems = clearanceItems.filter(i => i.is_sold);
  const unsoldItems = clearanceItems.filter(i => !i.is_sold);
  const itemsAtLoss = soldItems.filter(i => (i.profit_loss || 0) < 0).length;
  const itemsAtProfit = soldItems.filter(i => (i.profit_loss || 0) >= 0).length;
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Track clearance inventory and liquidation performance
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-xs text-muted-foreground">
              {stats.totalSold} cleared • {stats.totalUnsold} pending
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Category breakdown and liquidation metrics</CardDescription>
              </div>
              {showInsights ? (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="p-3 rounded-lg border bg-card">
                    <div className="text-sm text-muted-foreground mb-1">Items at Loss</div>
                    <div className="text-xl font-bold text-red-600">{itemsAtLoss}</div>
                    <div className="text-xs text-muted-foreground">sold below cost</div>
                  </div>
                  <div className="p-3 rounded-lg border bg-card">
                    <div className="text-sm text-muted-foreground mb-1">Items at Profit</div>
                    <div className="text-xl font-bold text-green-600">{itemsAtProfit}</div>
                    <div className="text-xs text-muted-foreground">sold above cost</div>
                  </div>
                  <div className="p-3 rounded-lg border bg-card">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium text-orange-900">
                  {stats.totalUnsold} items pending clearance
                </div>
                <div className="text-sm text-orange-700">
                  Total inventory value: {formatCurrency(unsoldItems.reduce((sum, i) => sum + i.cost_price, 0))}
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">No Clearance Items</h3>
              <p className="text-muted-foreground text-sm">
                Mark inventory items for clearance in the inventory page
              </p>
            </div>
          ) : (
              ...
              <div className="px-4 pt-4">
                <TabsList className="grid w-full grid-cols-2 max-w-xs">
                  <TabsTrigger value="cleared">
                    Cleared ({stats.totalSold})
                  </TabsTrigger>
                  <TabsTrigger value="pending">
                    Pending ({stats.totalUnsold})
                  </TabsTrigger>
                </TabsList>
              </div>
                          ...
                          {soldItems.slice(0, 50).map((item) => (
                            <tr key={item.id} className="border-b hover:bg-muted/30">
                              ...
                              <td className="py-2 px-2 text-right">
                                <span className={`font-semibold ${(item.profit_loss || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                  {(item.profit_loss || 0) >= 0 ? '+' : ''}{formatCurrency(item.profit_loss || 0)}
                                </span>
                              </td>
                ...
                <div className="p-4">
                  {unsoldItems.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p>All clearance items have been sold!</p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
