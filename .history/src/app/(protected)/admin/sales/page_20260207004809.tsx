...
export default function SalesPage() {
  ...
  const loadSales = async () => {
    try {
      ...
      let filteredData = (data || []) as SaleForList[];
      if (saleTypeFilter !== 'all' && data) {
        filteredData = (data as SaleForList[]).filter((sale) => {
      ...
      // Note: totalCount should reflect the server count when no client-side sale type filter
      // When sale type filter is applied, we filter client-side so count is from filtered data
      ...
      // Fetch stats from FULL filtered dataset using dedicated API function
