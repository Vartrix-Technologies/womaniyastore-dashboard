...
import { Search, Eye, Download, ArrowLeft, Receipt, TrendingUp, ChevronFirst, ChevronLast, ChevronLeft, ChevronRight, BarChart3, Users } from 'lucide-react';
...
export default function SalesPage() {
  ...
  // Use custom hooks
  const { dateFilter, setDateFilter, startDateISO } = useDateFilter({ initialFilter: 'week' });
  ...
  const loadSales = async () => {
    try {
      ...
      if (statsData && statsData.length > 0) {
        ...
        // Calculate sale-specific stats
        let festivalSales = 0, festivalRevenue = 0;
        let clearanceSales = 0, clearanceRevenue = 0;
        let promotionSales = 0, promotionRevenue = 0;
        
        statsData.forEach((sale: any) => {
          const items = sale.sale_items || [];
          items.forEach((item: any) => {
            if (item.sold_on_sale) {
              const finalPrice = item.final_price || 0;
              
              if (item.sale_type === 'festival') {
                festivalSales++;
                festivalRevenue += finalPrice;
              } else if (item.sale_type === 'clearance') {
                clearanceSales++;
                clearanceRevenue += finalPrice;
              } else if (item.sale_type === 'promotion') {
                promotionSales++;
                promotionRevenue += finalPrice;
              }
            }
          });
        });
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">View all sales transactions and bills</p>
          </div>
          <Button 
            onClick={exportSales}
            variant="outline"
      ...
      {(stats.festivalSales > 0 || stats.clearanceSales > 0 || stats.promotionSales > 0) && (
        <Card className="border-teal-200 bg-gradient-to-br from-teal-50/50 to-transparent">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-teal-600" />
              Sale Performance
            </CardTitle>
            <CardDescription>Breakdown of items sold on sale</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {stats.festivalSales > 0 && (
                <Link href="/admin/reports/festival" className="flex flex-col p-3 rounded-lg border border-green-200 bg-green-50/50 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-green-500"></span>
                    <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Festival</span>
                  </div>
                  <div className="text-2xl font-bold text-green-700">{stats.festivalSales}</div>
                  <div className="text-xs text-green-600 font-semibold mt-1">
                    {formatCurrency(stats.festivalRevenue)}
                  </div>
                  <div className="text-xs text-green-600 mt-2">View Report →</div>
                </Link>
              )}
              {stats.clearanceSales > 0 && (
                <Link href="/admin/reports/clearance" className="flex flex-col p-3 rounded-lg border border-red-200 bg-red-50/50 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-red-500"></span>
                    <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Clearance</span>
                  </div>
                  <div className="text-2xl font-bold text-red-700">{stats.clearanceSales}</div>
                  <div className="text-xs text-red-600 font-semibold mt-1">
                    {formatCurrency(stats.clearanceRevenue)}
                  </div>
                  <div className="text-xs text-red-600 mt-2">View Report →</div>
                </Link>
              )}
              {stats.promotionSales > 0 && (
                <div className="flex flex-col p-3 rounded-lg border border-blue-200 bg-blue-50/50">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-2 h-2 rounded-full bg-blue-500"></span>
                    <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Promotion</span>
                  </div>
                  <div className="text-2xl font-bold text-blue-700">{stats.promotionSales}</div>
                  <div className="text-xs text-blue-600 font-semibold mt-1">
                    {formatCurrency(stats.promotionRevenue)}
                  </div>
                </div>
              )}
            </div>
