  ...
  const loadSales = async () => {
    try {
      ...
      if (debouncedSearchTerm) {
        ...
        if (isNumericSearch) {
          // For numeric searches, check bill_number (exact/prefix match) OR name/phone
          // bill_number is BIGINT, so we use gte/lte for prefix-style matching
          const searchNum = parseInt(searchLower, 10);
          const searchNumMax = parseInt(searchLower + '9'.repeat(14 - searchLower.length), 10);
          query = query.or(`customer_name.ilike.%${searchLower}%,customer_phone.ilike.%${searchLower}%`);
          // Also filter for bill_number range if numeric (prefix match simulation)
          if (searchLower.length <= 14) {
            query = query.or(`bill_number.gte.${searchNum},bill_number.lte.${searchNumMax}`);
          }
        } else {
          // For text searches, only search name and phone
          query = query.or(`customer_name.ilike.%${searchLower}%,customer_phone.ilike.%${searchLower}%`);
        }
      }
