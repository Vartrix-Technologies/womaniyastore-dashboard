...
async function handleToggleActive(supabaseAdmin: any, user_id: string) {
  ...
  // Ban or unban the user at auth level using Admin API
  const { error: banError } = await supabaseAdmin.auth.admin.updateUserById(user_id, {
    ban_duration: newStatus ? 'none' : '876600h' // ~100 years ban if deactivating
  })

  if (banError) {
    console.error('Error banning/unbanning user:', banError)
    return new Response(
      JSON.stringify({ error: banError.message, code: 'BAN_ERROR' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
  ...
  if (updateError) {
    // Try to rollback the ban status
    await supabaseAdmin.auth.admin.updateUserById(user_id, {
      ban_duration: newStatus ? '876600h' : 'none'
    })
    return new Response(
