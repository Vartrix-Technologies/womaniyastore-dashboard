...
export async function getChecklistCompletionHistory(
  ...
  startDate: string,
  endDate: string
): Promise<ChecklistCompletionHistoryItem[]> {
  const { data, error } = await supabase
    .from('checklist_instances')
    .select(`
      *,
  ...
  if (error) throw error;
  return (data ?? []) as unknown as ChecklistCompletionHistoryItem[];
}

/**
 * Get completion stats for a checklist
 */
export async function getChecklistStats(checklistId: string, days: number = 30): Promise<ChecklistStats> {
