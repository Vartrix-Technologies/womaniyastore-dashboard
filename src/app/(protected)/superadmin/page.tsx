...
export default function SuperadminDashboard() {
  ...
  const handleExportDatabase = async () => {
    ...
    try {
      ...
      // Export all collections in parallel for faster downloads
      const [
        ...
        supabase.from('inventory_items').select('*').eq('shop_id', profile.shop_id),
        supabase.from('qr_codes').select('*').eq('shop_id', profile.shop_id),
        supabase.from('sales').select('*').eq('shop_id', profile.shop_id),
        supabase.from('profiles').select('*').eq('shop_id', profile.shop_id),
      ]);
      ...
      const safeName = (shop.shop_name || 'unknown').replace(/[^a-zA-Z0-9]/g, '-').replace(/-+/g, '-');
      const blob = new Blob([JSON.stringify(exportData, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${appConfig.internal.backupFilePrefix}-${safeName}-${new Date().toISOString().split('T')[0]}.json`;
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <Tabs defaultValue="system" className="w-full">
        ...
        <CardContent className="space-y-4">
          {systemStatus.map((item, idx) => (
            <div
              key={item.label}
              className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border rounded-lg gap-3 animate-stagger-fade-in"
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {quickActions.filter((a) => !a.disabled).map((action) => {
              ...
              <Card
                ...
                className={
                  action.disabled 
                    ...
                    : isClickable
                    ? `cursor-pointer hover:shadow-lg ${s.btnAnimation}`
                    : ''
                }
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => setConfirmDialog({
                    open: true,
                    title: 'Force Sync All Pending Sales',
                    description: `Are you sure you want to force sync ${pendingCount} pending sale(s)? This will push all pending data to the server.`,
