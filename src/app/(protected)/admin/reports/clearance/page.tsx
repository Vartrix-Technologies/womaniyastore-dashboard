    ...
    <div className="space-y-4 md:space-y-6 pb-24">
      ...
      <div className="flex items-center gap-3">
        <Link href={backUrl} className="relative group shrink-0">
          <div className="p-2.5 rounded-lg bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-md group-hover:shadow-lg transition-shadow">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Clearance Report</h1>
          <p className="text-sm text-muted-foreground">Analysis of items sold through clearance sales</p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className={`text-2xl font-bold ${stats.recoveryRate >= 100 ? 'text-green-600' : 'text-orange-600'}`}>
              <CountUp end={stats.recoveryRate} suffix="%" decimals={1} />
            </div>
            <div className="text-xs text-muted-foreground">
              of cost recovered
            </div>
        ...
        <Card className={stats.totalProfitLoss >= 0 ? 'border-green-200 bg-green-50/30' : 'border-red-200 bg-red-50/30'}>
          <CardContent className="p-4">
            ...
            <div className={`text-2xl font-bold ${stats.totalProfitLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              <CountUp end={Math.abs(stats.totalProfitLoss)} prefix={stats.totalProfitLoss < 0 ? '-₹' : '₹'} />
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {clearanceItems.slice(0, 50).map((item, idx) => (
                    <tr key={item.id} className="border-b hover:bg-muted/30 animate-stagger-fade-in" style={{ '--row-index': idx } as React.CSSProperties}>
