...
export function SyncStatusIndicator({
  ...
  const getStatusIcon = () => {
    ...
    if (syncing || inventoryCaching) return <Loader2 className="h-4 w-4 animate-spin" />;
  ...
  const getStatusColor = () => {
    ...
    if (syncing || inventoryCaching) return 'text-blue-500';
  ...
  const getTooltipContent = () => {
    ...
    if (inventoryCaching) {
      lines.push('Caching inventory for offline use...');
    } else if (lastInventoryCacheTime) {
      const ago = Math.round((Date.now() - new Date(lastInventoryCacheTime).getTime()) / 60000);
      lines.push(`Offline cache: synced ${ago < 1 ? 'just now' : `${ago}m ago`}`);
    }
    return lines;
  };
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              disabled={syncing || inventoryCaching}
            >
              {getStatusIcon()}
              {totalPending > 0 && (
