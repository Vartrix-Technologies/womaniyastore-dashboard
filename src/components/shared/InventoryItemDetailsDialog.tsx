...
import { Package, Calendar, Tag, DollarSign, Layers, ShieldCheck, ShoppingCart, Pencil, Trash2 } from 'lucide-react';
...
interface InventoryItemDetailsDialogProps {
  ...
  onDelete?: (item: InventoryItemForList) => void;
...
export function InventoryItemDetailsDialog({ item, onClose, onEdit, onDelete, onAddToCart }: InventoryItemDetailsDialogProps) {
  if (!item) return null;
        ...
        <div className="shrink-0 border-t px-5 py-3 space-y-2">
          ...
          {((onEdit && canEdit) || (onDelete && canDelete)) && (
            <div className="flex gap-2">
              {onEdit && canEdit && (
                <Button onClick={() => onEdit(item)} variant="secondary" className="flex-1">
                  <Pencil className="mr-2 h-4 w-4" />
                  Edit
                </Button>
              )}
              {onDelete && canDelete && (
                <Button onClick={() => onDelete(item)} variant="outline" className="flex-1 text-destructive hover:bg-destructive/10 hover:border-destructive">
