...
export function QrPrefixesTab({ shopId, qrPrefixes, onRefresh }: QrPrefixesTabProps) {
  ...
  // Field errors
  const { errors, validateFields, clearFieldError } = useFormErrors<'prefix'>();
  ...
  const handleSave = async () => {
    ...
    const valid = validateFields({
      prefix: [
        !prefix
          ? true
          : !validateFormat(prefix),
        !prefix ? 'Prefix is required' : 'Use uppercase alphanumeric with hyphens (e.g. WA-499)',
      ],
    });
    if (!valid) return;
    try {
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          {qrPrefixes.length === 0 ? (
            <EmptyState
              icon={QrCode}
              title="No prefixes yet"
              description="Create your first prefix to start organizing QR-coded inventory"
              action={
                <Button
                  size="sm"
                  onClick={openAdd}
              ...
              {qrPrefixes.map((qr) => (
                <div
                  ...
                  className={`relative rounded-xl border-2 p-4 transition-all ${
                    qr.is_active
                      ? `${a.border} ${a.borderDark} ${a.bgSubtle} ${a.bgDarkSubtle}`
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input
                value={form.prefix}
                onChange={(e) => { setForm({ ...form, prefix: e.target.value.toUpperCase() }); clearFieldError('prefix'); }}
                onKeyDown={(e) => e.key === 'Enter' && handleSave()}
                placeholder="e.g. WA-499"
                className={`font-mono h-11 text-base font-semibold tracking-wider ${fieldErrorClass(errors.prefix)}`}
                autoFocus
              />
